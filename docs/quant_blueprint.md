...
Owner: Solo Dev (Lian Isaac) | Date: 2025-10-18 | Version: QE v1.0 (P2 95% → P3 Prep)  

## I. System Layers (Flow: Scraper → DB → Metrics → WS → UI)  
| Layer | Function | Modules (Files) |  
|-------|----------|-----------------|  
| 1. Ingestion | High-freq pulls (OI/LS/price via CCXT/WS; jitter<10/sec) | scraper.py (Semaphore(8)/30s queue/tf rotate 5m-15m); metrics.py (Coingecko mock/guards) |  
| 2. Persistence | Bulk store + tf bind (SQLite → DuckDB P3 tease) | db.py (upsert cols: oi_abs_usd/global_ls/top_ls/imbalance/funding/cvd/rsi/z_score/timeframe; finite np.isfinite) |  
| 3. Analytics | Pre-calc quants (deltas/Z/RSI/CVD; weighted OI P3) | metrics.py (get_all batch; quant_utils inline: Z=(x-μ)/σ roll20; corr Pearson) |  
| 4. API/Event | Paginated REST + WS emit (batch/tf ?arg) | app.py (/api/metrics Content-Range; socketio.emit 'metrics_update'/alert_toast) |  
| 5. Viz/UX | Table/modal (9+ lines) + tf switch/toast/export | App.jsx (WS debounce1s/LocalForage per-tf; TradingChart Recharts; hot-toast Z-spike) |  
| 6. Intelligence (P4) | Replay/corr/sim (bisect joins + PnL regret) | /api/replay tease; trade_sim.py (OBV/RSI entry/exit log JSON) |  

## II. Core Calculations (Inline Formulas – Drop to metrics.py)  
| Metric | Formula | Purpose | Snippet Test (code_execution) |  
|--------|---------|---------|-------------------------------|  
| OI Δ% | (OI_t - OI_{t-1}) / OI_{t-1} * 100 | Leverage flow | `import numpy as np; oi = np.array([1e9, 1.01e9]); delta = (oi[1]-oi[0])/oi[0]*100; print(delta)` → 1.0 |  
| Global LS | Long Vol / Short Vol | Sentiment skew | In db.py: `ls_ratio = long_vol / short_vol if short_vol else 1` |  
| Imbalance | (Buy - Sell) / Total Vol | Microstructure bias | `imb = (buy - sell) / (buy + sell); np.isfinite(imb)` → Guard <50% |  
| CVD | Σ(Buy - Sell) over tf | Volume confirmation | Rolling in metrics.py: `df['cvd'] = df['buy'].cumsum() - df['sell'].cumsum()` |  
| RSI (14) | 100 - 100/(1 + RS); RS=AvgGain/AvgLoss | Momentum revert | TA-lib tease: `from ta.momentum import RSIIndicator; rsi = RSIIndicator(close).rsi()` |  
| Z-Score | (x - μ) / σ (roll 20 bars/tf) | Anomaly detect | `from scipy.stats import zscore; z = zscore(df['oi_delta_pct'].rolling(20).mean())` → <10 cap |  
| Weighted OI (P3) | Σ(OI * vol) / Σ(vol) | Exch aggregate | `weights = df['vol']; w_oi = np.average(df['oi'], weights=weights/weights.sum())` |  
| Confluence (P3) | Sum(Z>2.5 + LS>2 + |imb|>3%) / 3 | Multi-factor alert | `score = sum([z>2.5, ls>2, abs(imb)>3])/3; if score>0.66: emit 'alert_toast'` |  
| Corr (P4) | cov(OI, Price) / (σ_OI * σ_Price) roll24h | Confluence r | `from scipy.stats import pearsonr; r, _ = pearsonr(df['oi'], df['price'])` → Heatmap JSON |  
