...
def generate_mock(tf='5m'):  
    ...
    for sym in symbols:  
        ...
        # Z-score calc tease (mean from base ls, std 0.3 for variance)  
        z_ls = (global_ls - base['global_ls']) / 0.3  # Simple Z from jitter  
        m = {  
            'symbol': sym,  
...
async def main():  
    ...
    with temp_app.app_context():  
        ...
        metrics = generate_mock('5m')  # Scaled mocks w/ tf bind  
        saved = save_metrics(metrics, timeframe='5m')  # Save w/ tf param  
        print(f"Seeded {len(metrics)} valid pairs, saved {saved} to DB")  
        if metrics:  
            print("Sample last row:", metrics[-1])  # e.g., {'symbol': 'PEPE', 'z_score': -0.86, 'timeframe': '5m', ...}  
        import sqlite3  
